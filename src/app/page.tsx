...
export default function HomePage() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      setOauth((await res.json()) as OAuthStatus);
    } catch {
      setOauth(null);
    }
  }, []);
  ...
  async function ask() {
    ...
    try {
      ...
      const payload = (await res.json()) as { data?: AskResult; error?: string };
      if (!res.ok || !payload.data) throw new Error(payload.error || `请求失败 HTTP ${res.status}`);
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-12">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-[12px]">
        ...
        <span className="text-ink-300">{oauth?.note}</span>
      </div>
...
/** 步骤名 → 中文标签。顺序与 src/steps/index.ts 里的声明一致。 */
