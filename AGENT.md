...
10. **`src/front/` must not `import @/back/*`, and must not call `fetch` directly.** It may only use `@/front/api-client` (the single request exit) and `@/shared/contract` (types). Self-check:
    ...
    ```
11. **Response shapes are owned by the frontend contract, not chosen by us.** Success: the **payload is the body itself** (no `{status,data}` wrapper). Failure: `{ code, message, retryable, details? }`. Build them with `ok()` / `fail()` in `src/back/handlers/types.ts` — never hand-assemble. The frontend validates with zod `.strict()`, so **one extra key invalidates the whole response** and the backend logs nothing. `e2e` asserts exact key sets per endpoint.
...
12a. **Never parse a query param with `Number(x)` and clamp it.** `Number(null) === 0` (a *finite* number), so a default-then-clamp turns "absent" into `1`. This silently truncated `/api/conversations/:id/messages` to one message. Use `pageLimit()` from `src/back/framework/params.ts`.
...
16. **The conversation domain must not require login.** The frontend's `/app/chat/:conversationId` is the *only* route without `RequireAuth` — every other `/app/*` route has it. Returning 401 would make that page dead on arrival. Unauthenticated callers get a stable guest identity from the `zh_guest` cookie (see `src/back/handlers/conversations.ts`).
17. **The conversation agent may only quote evidence already on the creator card.** It does not run its own retrieval. When `creator.evidence` is empty, do not call the model at all — reply with the honest "I have nothing citable" path. A fluent answer with no provenance is the one failure this product cannot afford.
18. **Streaming must fail loudly inside the stream.** Once a `200` + NDJSON response starts, the status code cannot change, so every failure path must emit a terminal event (`run.failed` / `agent.run.failed`) with a contract error code. Also flush each line immediately and send `X-Accel-Buffering: no` — buffering anywhere in the middle silently disables streaming.
