...
import { extractJson, structureSystem } from '@/framework/llm-utils';
import type { LlmClient, QuotaGuard } from '@/framework/ports';
...
abstract class ChatCompletionsLlm implements LlmClient {
  ...
  async complete(req: {
    ...
    const { value } = await this.opts.cache.getOrSet(
      `${this.name}:complete:${key}:${JSON.stringify(payload)}`,
      this.ttl(),
      run,
    );
    return value;
