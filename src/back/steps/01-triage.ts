...
import type { TriageResult } from '@/back/domain/types';
...
export const triageStep: Step = {
  ...
  // from: null → 输入就是流水线的启动输入（AskBoot），不是裸字符串
  cacheKey: (input) => `triage:${String((input as AskBoot)?.question ?? input).slice(0, 200)}`,
  async run(boot: AskBoot, ctx) {
