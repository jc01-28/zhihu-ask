...
import { DiskCache } from '@/back/framework/cache';
import { dataDir } from '@/back/framework/datadir';
import type { ContextDeps, Logger } from '@/back/framework/pipeline';
import { DiskQuotaGuard } from '@/back/framework/quota';
import type { ContentSource, LlmClient } from '@/back/framework/ports';
...
/**
 * 缓存命名空间 —— 把「会改变结果的运行时环境」编进所有流水线缓存 key。
 *
 * 为什么必须有：缓存是**落盘且跨进程共享**的（`.cache/`）。不加这段就会出现：
 * 先跑一轮 `FIXTURE_CORPUS=real`，再切到 synthetic —— 结果仍然来自真实语料，
 * 而每一步都命中缓存、trace 全绿，看起来完全正常。
 *
 * 实测踩到过：三组模式验证里，B（在线直连）和 C（合成语料）整条链路全 cached，
 * 直接复用了 A（真实语料）的结果 —— C 的 health 报 174 条合成语料，
 * 却返回真实语料的作者，自相矛盾才暴露出来。所以数据源 + 语料范围必须进 key。
 */
