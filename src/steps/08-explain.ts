...
export const explainStep: Step = {
  ...
  async run(
    ...
    const question = input[SYS_INPUT];
    ...
    const verified = input.verified ?? {
      candidates: [],
      stats: { totalEvents: 0, traceableEvents: 0, droppedCandidates: 0 },
    };
    ...
    const recommendations: Recommendation[] = verified.candidates
      ...
      .map((candidate, index) => ({
        ...
        verified: candidate.experiences.length > 0,
      }));
    ...
    ctx.logger.info(
      `生成 ${recommendations.length} 张推荐卡片，证据覆盖率 ${(evidenceCoverage * 100).toFixed(0)}%`,
    );
